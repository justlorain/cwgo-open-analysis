# CWGO-Open-Analysis (*This is a community driven project*)

A service for analyzing cloudwego open-source community github data.
...
```shell
go install github.com/cloudwego-contrib/cwgo-open-analysis@latest
```

- Get Library

```shell
go get -u github.com/cloudwego-contrib/cwgo-open-analysis
```
...
- **[Start](#start---start-cwgo-open-analysis-service)**: Start cwgo-open-analysis service
- **[Restart](#restart---restart-cwgo-open-analysis-service)**: Restart cwgo-open-analysis service

```shell
Usage:        
  cwgo-open-analysis [command]

Available Commands:
  help        Help about any command
  restart     restart cwgo-open-analysis service
  start       start cwgo-open-analysis service

Flags:
  -h, --help      help for cwgo-open-analysis
  -v, --version   version for cwgo-open-analysis
```
...
```shell
cwgo-open-analysis start [flags] [path2config]
```
...
```shell
cwgo-open-analysis start -c "@hourly" -r "5" config.yaml
```
...
```shell
cwgo-open-analysis restart [flags] [path2config]
```
...
```shell
cwgo-open-analysis restart -t "example-github-token"
```
...
Config the command in the `cwgo-open-analysis` service.
...
```shell
docker build -t cwgo-open-analysis .
```

```shell
docker run -it -v path2config.yaml:/src cwgo-open-analysis start path2config.yaml
```
